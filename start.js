 ...
 * ----------------------
 * This is the "Start Command" Hostinger's Node.js panel runs.
 *
 * It does 3 things automatically:
 *   1. If .next/standalone/ doesn't exist → run `npm run build` first
 *   2. If .env doesn't exist → create it from .env.example with a generated JWT secret
 *   3. Start the production server from .next/standalone/server.js
 *
 * The user just needs to:
 *   - Upload the zip
 *   - Extract it
 *   - Set Start Command to: node start.js
 *
 * Everything else happens automatically on first start.
 */
...
function ensureEnvFile() {
  ...
  if (!fs.existsSync(ENV_EXAMPLE_PATH)) {
    // Create a default .env if even .env.example is missing
    ...
    log('⚠️  Edit .env to add your ZAI_API_KEY for AI features.');
    return;
  }
